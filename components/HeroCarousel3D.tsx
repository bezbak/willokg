...
export function HeroCarousel3D() {
  ...
  const transform = useTransform(smoothRotation, (value) => `translateZ(clamp(-360px, -28vw, -180px)) rotateY(${value}deg)`);
    ...
    <div className="hero-stage" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}>
      <motion.div className="hero-ring" style={{ transform }}>
        {heroCards.map((card, index) => (
          <article key={card.title} className="hero-card" style={{ transform: `rotateY(${index * STEP}deg) translateZ(clamp(260px, 38vw, 560px))` }}>
            <span className="card-fallback" />
            <SmartImage src={card.image} alt={card.title} fill sizes="(max-width: 768px) 34vw, 180px" className="object-cover" priority={index < 3} />
          </article>
        ))}
      </motion.div>
    </div>
